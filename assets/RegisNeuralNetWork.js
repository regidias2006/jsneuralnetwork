...
this.ActivationFunctions = function (aFunc, input) {
    ...
    this.sigmoid = function(x){
        return 1.0 / (1.0 + Math.exp(-x)); //validar calculo da sigmoid
    }
...
this.MultiLayerPerceptron = function(){
    ...
    this.Init = function(layerSizes, activationFunctions){
        if(activationFunctions.length != layerSizes.length || activationFunctions[0] != ActivationFunction.None) {
            console.log("Cannot construct a network with these parameters.")
            return;
        }
        ...
        for (let l = 0; l < layerCount; l++) {
            ...
            for (let i = 0; i < (l == 0 ? inputSize : layerSize[l - 1]); i++) {
                weight[l][i] = [];
    ...
    this.Run = function(input){
        ...
        for (let i = 0; i < layerSize[layerCount - 1]; i++) {
    ...
    this.TrainCase = function(input, desired, TrainingRate, Momentum) {
        ...
        for (let l = layerCount - 1; l >= 0; l--) {
            if (l == layerCount - 1) {
                for (let k = 0; k < layerSize[l]; k++) {
                    delta[l][k] = output[k] - desired[k];
                    error += Math.pow(delta[l][k], 2);
                    delta[l][k] *= new ActivationFunctions().EvaluateDerivative(activationFunction[l], layerSize[l][k])                    
                }
            }
            else //Hidden Layer
            {
    ...
    this.Train = function(training_sets, max_count) {
        ...
        for (let i = 0; i < training_sets.length; i++) {
            ...
            for (let j = 0; j < training_sets[i].input.length; j++) {
        ...
        for (let i = 0; i < training_sets.length; i++) {
            ...
            for (let j = 0; j < training_sets[i].output.length; j++) {
...
=======
                error += Network.TrainCase(input[i], output[i], 0.32, 0.055);
            }

            if(count % 100 == 0) console.log("Ephoc " + count + " completed with error " + error);
            //if(count == 1 && error > 17.00 && error < 20.00) error = 0001
>>>>>>> aadb99c4a5f9d16a171c2b2bac7695de0eb99ef4
        } while (error > 0.0001 &&  count <= max_count);
    }

    this.ToJson = function(o){
        var writer = {
            NeuralNetwork: { Type: "MultiPerceptron"},
